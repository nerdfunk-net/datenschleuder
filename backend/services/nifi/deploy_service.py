...
def deploy_flow(instance_id: int, deployment_data: dict) -> dict:
    ...
    logger.info("Deployment data: %s", json.dumps(deployment_data, indent=2, default=str))
    
    instance = _instance_repo.get_by_id(instance_id)
    if not instance:
        raise ValueError("NiFi instance with ID %d not found" % instance_id)
    
    logger.info("Target Instance: %s (hierarchy: %s=%s)", 
                instance.name, instance.hierarchy_attribute, instance.hierarchy_value)
    ...
    if template_id:
        ...
        logger.info("Template resolved: name=%s, bucket_id=%s, flow_id=%s, registry_id=%s",
                   template_name, bucket_id, flow_id, registry_client_id)
        ...
        logger.info("Using direct registry info: bucket_id=%s, flow_id=%s, registry_id=%s",
    ...
    logger.info("Requested version: %s", requested_version if requested_version else "latest")
    ...
    logger.info("="*60)
    original_name = deployed_pg.component.name if hasattr(deployed_pg, "component") else "unknown"
    ...
    if pg_id and parameter_context_name:
        ...
        try:
            ...
            logger.info("Parameter context '%s' assigned successfully to PG %s", parameter_context_name, pg_id)
        except Exception as e:
            logger.error("Failed to assign parameter context by name: %s", e, exc_info=True)
            raise
        ...
        try:
            ...
            logger.info("Parameter context ID '%s' assigned successfully to PG %s", parameter_context_id, pg_id)
        except Exception as e:
            logger.error("Failed to assign parameter context by ID: %s", e, exc_info=True)
            logger.warning("Could not assign parameter context: %s", e)
    else:
        logger.info("No parameter context to assign (parameter_context_name=%s, parameter_context_id=%s)",
                   parameter_context_name, parameter_context_id)

    logger.info("="*60)
    logger.info("STEP 9: Auto-Connect Ports")
    logger.info("="*60)
    if pg_id and resolved_parent_pg_id:
        logger.info("Auto-connecting ports for PG %s in parent %s", pg_id, resolved_parent_pg_id)
        try:
        ...
        logger.info("Skipping port auto-connection (pg_id=%s, parent_pg_id=%s)", pg_id, resolved_parent_pg_id)
    ...
    if pg_id and deployment_data.get("stop_versioning_after_deploy"):
        ...
        logger.info("Skipping version control stop (pg_id=%s, stop_versioning=%s)",
                   pg_id, deployment_data.get("stop_versioning_after_deploy"))
    ...
    if pg_id and deployment_data.get("disable_after_deploy"):
        ...
        logger.info("Skipping process group disable (pg_id=%s, disable_after_deploy=%s)",
                   pg_id, deployment_data.get("disable_after_deploy"))
    ...
    if pg_id and deployment_data.get("start_after_deploy"):
        ...
        logger.info("Skipping process group start (pg_id=%s, start_after_deploy=%s)",
                   pg_id, deployment_data.get("start_after_deploy"))
